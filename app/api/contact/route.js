...
export async function POST(request) {
  try {
    ...
    await transporter.sendMail({
      from: `"GRO - CND Logistic" <${process.env.SMTP_USER}>`,
      to: process.env.CONTACT_EMAIL || "sales@cndlogistic.com",
      replyTo: email,
      subject: subject
        ? `[Website GRO] ${subject}`
        : `[Website GRO] Pesan dari ${name}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; background: #f9fafb; border-radius: 10px;">
          <div style="background: #0e1630; padding: 20px 28px; border-radius: 8px 8px 0 0;">
            <h2 style="color: #ffffff; margin: 0; font-size: 20px;">📬 Pesan Baru dari Website GRO</h2>
          ...
          <p style="color: #9ca3af; font-size: 11px; text-align: center; margin-top: 16px;">
            Dikirim melalui form kontak website GRO &bull; ${new Date().toLocaleString("id-ID", { timeZone: "Asia/Jakarta" })} WIB
          </p>
        </div>
      `,
    });
